...
import {
    ...
    Globe, Edit2, Trash2, Terminal, Cpu, Database, Cloud,
    Code2, ShieldCheck, Box
} from "lucide-react";
...
const SECTIONS = [
    ...
    { id: "system", name: "System Software", icon: Terminal },
    { id: "application", name: "Applications", icon: Box },
    { id: "development", name: "Dev Tools", icon: Code2 },
...
const UNIFIED_HEADERS = [
    ...
    { label: "SOFTWARE NAME", key: "softwareName", icon: Box },
    { label: "VERSION", key: "version", icon: Hash },
    { label: "DEVELOPER", key: "developer", icon: User },
    { label: "FIRST RELEASE", key: "firstRelease", icon: Calendar },
    { label: "LICENSE KEY", key: "licenseKey", icon: ShieldCheck },
    { label: "EXPIRY DATE", key: "expiryDate", icon: Calendar },
    { label: "PURCHASE DATE", key: "purchaseDate", icon: Calendar },
...
export default function Softwares() {
    ...
    const [selectedSoftwareType, setSelectedSoftwareType] = useState("system");
    ...
    const fetchSummary = async () => {
        try {
            const res = await fetch("/api/softwares/summary");
            const data = await res.json();
    ...
    const fetchTableData = useCallback(async (section: string) => {
        ...
        await withLoading(async () => {
            try {
                const res = await fetch(`/api/softwares/${encodeURIComponent(section)}`);
                const data = await res.json();
            ...
            try {
                const res = await fetch("/api/asset-requests", {
                    ...
                    body: JSON.stringify({
                        type: action,
                        category: "Softwares",
                        section: editingId ? activeSection : selectedSoftwareType,
                        assetData: editingId ? { ...formData, id: editingId } : formData
                    }),
                });
    ...
    if (userRole === "employee") {
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
                ...
                <p className="text-gray-500 font-medium text-center">You do not have permission to view Softwares.</p>
            </div>
        );
    }
            ...
            <div className="flex-1 overflow-y-auto p-6 max-w-7xl mx-auto w-full space-y-8">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="flex items-center gap-3">
                        <div className="p-4 bg-blue-600 text-white rounded-2xl shadow-lg shadow-blue-500/10">
                            {(() => { const SectionIcon = SECTIONS.find(s => s.id === activeSection)?.icon || Box; return <SectionIcon size={28} />; })()}
                        </div>
                        <div>
                            <h2 className="text-3xl font-black text-gray-900 dark:text-white uppercase tracking-tight">{activeSection === 'overall' ? 'SOFTWARE INVENTORY' : SECTIONS.find(s => s.id === activeSection)?.name}</h2>
                            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
                                {activeSection === "overall" ? `Tracking Software Assets` : `Total ${summaryCounts[activeSection] || 0} items`}
                            </p>
                        </div>
                    </div>
                    <button
                        onClick={() => {
                            ...
                            else if (!editingId) setSelectedSoftwareType(activeSection === "overall" ? "system" : activeSection);
                        }}
                    ...
                    >
                        ...
                        {isAddEntryOpen ? "Close Panel" : "Register Software"}
                    </button>
                </div>
                ...
                {isAddEntryOpen && (
                    <div className="bg-white dark:bg-card rounded-3xl border-2 border-blue-500/10 p-8 shadow-2xl animate-in fade-in zoom-in-95 duration-300">
                        <h3 className="text-2xl font-black text-gray-900 dark:text-white mb-8 border-l-4 border-blue-600 pl-4">{editingId ? 'Modify' : 'Register New'} Software</h3>
                        <form onSubmit={handleSubmit} className="space-y-8">
                                ...
                                {!editingId && (
                                    <div className="md:col-span-1 lg:col-span-1">
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => { setSelectedSoftwareType(e.target.value); setFormData({}); }}
                            ...
                            <button type="submit" className="px-12 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-xl hover:-translate-y-1 transition-all active:scale-95">
                                {editingId ? "Update Software" : "Complete Registration"}
                            </button>
                        </form>
                    </div>
                )}
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3">
                        {SECTIONS.filter(s => s.id !== 'overall').map((section, idx) => (
                            <AssetCard
                                ...
                                variant={COLORS[idx % COLORS.length] as "blue" | "green" | "orange" | "purple" | "red" | "indigo" | "pink"}
                                Icon={section.icon}
                                size="sm"
                                onClick={() => setActiveSection(section.id)}
                            />
                    ...
                    <div className="bg-white dark:bg-card rounded-[2.5rem] border dark:border-gray-800 shadow-sm overflow-hidden min-h-[400px]">
                        <div className="overflow-x-auto">
                            <table className="w-full border-collapse">
                                ...
                                <tbody>
                                    {tableData.length === 0 ? (
                                        <tr><td colSpan={UNIFIED_HEADERS.length + 1} className="px-6 py-20 text-center text-gray-400 font-bold uppercase tracking-widest italic opacity-50">No softwares found in this category</td></tr>
                                    ) : (
                                        tableData.map((item, idx) => (
                                            <tr key={(item._id as string) || idx} className="border-b dark:border-gray-800/50 hover:bg-gray-50/50 dark:hover:bg-gray-800/20 transition-colors group">
                                                ...
                                                {UNIFIED_HEADERS.slice(1).map(h => (
                                                    <td key={h.key} className="px-6 py-4">
                                                        <span className={clsx("text-sm font-bold tracking-tight", h.key === 'softwareName' ? "text-blue-600 dark:text-blue-400" : "text-gray-900 dark:text-gray-200")}>
                                                            {String(item[h.key] || '-')}
                                                        </span>
                                                    </td>
                                                ))}
